...
class CreateUserApi:
    ...
    def __call__(self):
        try:
            ...
            user_dto = self.serializer_manager.get_dto(user_data)
            create_user = CreateUser(self.user_repository, self.password_generator, self.django_unit_of_work)
            create_user(**user_dto)
            response_data = dict(
                success=True,
                message='All ok',
            )
            return AbstractResponse(response_data, status=http_status.HTTP_201_CREATED)
            ...
            return AbstractResponse(response_data, status=http_status.HTTP_400_BAD_REQUEST)
